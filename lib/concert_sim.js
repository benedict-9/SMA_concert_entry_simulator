...
function loop(wallTime) {
  ...
  // change to stop when all attendees have entered
  const done = attendees.every(a => a.state === S.ENTERED);
  if (done) {
    isRunning = false;
    document.getElementById('clock').textContent += '  ✓ Done';
    // keeps animating until all dots reach their final positions
    function finishAnimation() {
      computeTargets();
      lerpDots();
      const settled = attendees.every(a =>
        a.vx === null || (Math.abs(a.vx - a.tx) < 0.5 && Math.abs(a.vy - a.ty) < 0.5)
      );
      updateDots();
      drawServerPulses();
      if (!settled) requestAnimationFrame(finishAnimation);
    }
...
// ═══════════════════════════════════════════════════════════
//  BATCH RUN ENGINE  (headless DES — no rendering)
// ═══════════════════════════════════════════════════════════

/**
 * Run one complete simulation with the given config snapshot.
 * Returns KPIs for that replication.
 */
function runOneSim(cfg) {
  // ── local state (no globals touched) ──
  let t = 0;
  const DT = 1; // advance 1 sim-second per step

  // spawn attendees
  const pax = [];
  for (let i = 0; i < cfg.totalAttendees; i++) {
    const isVIP = Math.random() < cfg.vipPct;
    pax.push({
      isVIP,
      hasBag:     Math.random() < cfg.bagPct,
      needsWrist: Math.random() < cfg.wristPct,
      wantsWater: Math.random() < cfg.waterPct,
      arrivalTime: cfg.arrivalMode === 'nhpp'
        ? randArrivalNHPP(cfg.arrivalWindow)
        : randArrivalPoisson(cfg.arrivalWindow),
      state: 'arriving',
      queuedAt: null,
      enteredAt: null,
    });
  }
  pax.sort((a, b) => a.arrivalTime - b.arrivalTime);

  // queues
  let holdQ = [], bagVIP = [], bagNorm = [], wristQ = [], waterQ = [];

  // servers: { busyUntil, current }
  const mkSrv = n => Array.from({length: n}, () => ({ busyUntil: 0, current: null }));
  const bagSrv   = mkSrv(cfg.bagLanes + 1);
  const wristSrv = mkSrv(cfg.wristCounters);
  const waterSrv = mkSrv(cfg.waterStations);
  ...
  // routing helpers
  function afterBag(p) {
    if (p.needsWrist) { p.state = 'wq'; wristQ.push(p); }
    else afterWrist(p);
  }
  function afterWrist(p) {
    if (p.wantsWater) { p.state = 'waq'; waterQ.push(p); }
    else afterWater(p);
  }
  function afterWater(p) {
    p.state = 'done';
    p.enteredAt = t;
    if (p.queuedAt !== null) waits.push((p.enteredAt - p.queuedAt) / 60);
  }

  // generic server tick
  function tickServer(srv, priQ, secQ, isVIPLane, svcFnPri, svcFnSec) {
    if (srv.current && t >= srv.busyUntil) {
      const done = srv.current; srv.current = null;
      if (isVIPLane !== undefined) afterBag(done);
      else if (srv._type === 'wrist') afterWrist(done);
      else afterWater(done);
    }
    if (!srv.current) {
      let next = null, svcFn = svcFnPri;
      if (priQ.length) { next = priQ.shift(); }
      else if (secQ && secQ.length && (isVIPLane ? cfg.vipSpillover : true)) {
        next = secQ.shift(); svcFn = svcFnSec || svcFnPri;
      }
      if (next) { next.state = 'svc'; srv.current = next; srv.busyUntil = t + svcFn(); }
    }
  }

  // tag wrist/water servers so afterBag callback knows which to call
  wristSrv.forEach(s => { s._type = 'wrist'; });
  waterSrv.forEach(s => { s._type = 'water'; });

  // ── main loop ──
  const maxT = cfg.arrivalWindow * 60 + 7200; // window + 2hr buffer
  let steps = 0;
  while (steps < maxT && !pax.every(p => p.state === 'done')) {
    t += DT; steps++;

    // arrivals → holding
    for (const p of pax) {
      if (p.state === 'arriving' && p.arrivalTime <= t) {
        p.state = 'holding'; p.queuedAt = t; holdQ.push(p);
      }
    }

    // batch release
    if (t >= nextBatch && holdQ.length) {
      const rel = holdQ.splice(0, cfg.batchSize);
      for (const p of rel) {
        if (p.hasBag) { p.state = 'bq'; (p.isVIP ? bagVIP : bagNorm).push(p); }
        else afterBag(p);
      }
      nextBatch = t + cfg.batchInterval;
    }

    // bag check
    tickServer(bagSrv[0], bagVIP, bagNorm, true,  SVC.bagVIP,    SVC.bagNormal);
    for (let i = 1; i < bagSrv.length; i++)
      tickServer(bagSrv[i], bagNorm, null, false, SVC.bagNormal, null);

    // wristband
    for (const s of wristSrv) tickServer(s, wristQ, null, undefined, SVC.wristband, null);

    // water
    for (const s of waterSrv) tickServer(s, waterQ, null, undefined, SVC.water, null);
  }

  // ── KPIs ──
  const entered = pax.filter(p => p.state === 'done').length;
  const avgWait = waits.length ? waits.reduce((a,b)=>a+b,0)/waits.length : 0;
  const p95Wait = waits.length ? percentile(waits, 0.95) : 0;
  const p99Wait = waits.length ? percentile(waits, 0.99) : 0;
  const maxWait = waits.length ? Math.max(...waits) : 0;
  const throughput = entered / (t / 60); // pax per minute

  return { avgWait, p95Wait, p99Wait, maxWait, throughput, entered, totalTime: t / 60 };
}

/**
 * Run N replications headlessly, compute mean + 95% CI for each KPI.
 * Uses async/chunked execution so the browser doesn't freeze.
 */
async function runBatch(n, onProgress) {
  const results = [];
  const CHUNK = 10; // run 10 reps per frame

  const cfgSnap = { ...CFG }; // snapshot current config

  for (let i = 0; i < n; i += CHUNK) {
    const end = Math.min(i + CHUNK, n);
    for (let j = i; j < end; j++) {
      results.push(runOneSim(cfgSnap));
    }
    onProgress(end, n);
    await new Promise(r => setTimeout(r, 0)); // yield to browser
  }

  return summarise(results, cfgSnap);
}

const KPI_DEFS = [
  { key: 'avgWait',    label: 'Avg Wait (min)',    fmt: v => v.toFixed(2) },
  { key: 'p95Wait',    label: 'P95 Wait (min)',    fmt: v => v.toFixed(2) },
  { key: 'p99Wait',    label: 'P99 Wait (min)',    fmt: v => v.toFixed(2) },
  { key: 'maxWait',    label: 'Max Wait (min)',    fmt: v => v.toFixed(2) },
  { key: 'throughput', label: 'Throughput (pax/min)', fmt: v => v.toFixed(2) },
  { key: 'totalTime',  label: 'Total Sim Time (min)', fmt: v => v.toFixed(1) },
];

function summarise(results, cfg) {
  const n = results.length;
  // t-critical for 95% CI, df = n-1 (approx for large n)
  const tCrit = n >= 30 ? 1.96 : jStat.studentt.inv(0.975, n - 1);

  const stats = KPI_DEFS.map(kpi => {
    const vals = results.map(r => r[kpi.key]);
    const mean = vals.reduce((a,b)=>a+b,0) / n;
    const variance = vals.reduce((s,v)=>s+(v-mean)**2,0) / (n-1);
    const se = Math.sqrt(variance / n);
    const hw = tCrit * se; // half-width of CI
    return {
      ...kpi,
      mean, hw, se,
      min: Math.min(...vals),
  ...
  return { n, stats, cfg, raw: results };
}
...
function renderBatchResults(summary) {
  lastBatchResult = summary;
  const { n, stats, cfg } = summary;

  const cfgLine = [
    `n=${n}`,
    `attendees=${cfg.totalAttendees}`,
    `vip=${(cfg.vipPct*100).toFixed(0)}%`,
    `bags=${(cfg.bagPct*100).toFixed(0)}%`,
    `bagLanes=${cfg.bagLanes}`,
    `arrival=${cfg.arrivalMode}`,
    `batchSize=${cfg.batchSize}`,
    `batchInterval=${cfg.batchInterval}s`,
  ].join('  ·  ');

  let html = `<div class="batch-cfg-label">${cfgLine}</div>`;
  html += `<div class="batch-section-title">KPI Summary — ${n} replications, 95% CI</div>`;
  ...
  for (const s of stats) {
    html += `<tr>
      <td>${s.label}</td>
      <td>${s.fmt(s.mean)}</td>
      <td class="ci">± ${s.fmt(s.hw)}</td>
      <td>${s.fmt(s.min)}</td>
      <td>${s.fmt(s.max)}</td>
      <td class="ci">${s.fmt(s.se)}</td>
    </tr>`;
  }
  ...
  document.getElementById('batch-copy').style.display = 'inline-block';
}

function exportBatchCSV() {
  if (!lastBatchResult) return;
  const { n, stats, cfg, raw } = lastBatchResult;

  let csv = 'KPI,Mean,CI_halfwidth,Min,Max,SE\n';
  for (const s of stats) {
    csv += `${s.label},${s.mean.toFixed(4)},${s.hw.toFixed(4)},${s.min.toFixed(4)},${s.max.toFixed(4)},${s.se.toFixed(4)}\n`;
  }
  csv += '\nRaw replications\n';
  csv += 'rep,' + KPI_DEFS.map(k=>k.key).join(',') + '\n';
  raw.forEach((r, i) => {
    csv += `${i+1},` + KPI_DEFS.map(k => r[k.key].toFixed(4)).join(',') + '\n';
  });

  const blob = new Blob([csv], { type: 'text/csv' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `batch_n${n}_${cfg.arrivalMode}_lanes${cfg.bagLanes}.csv`;
  a.click();
}
...
function copyBatchTSV() {
  ...
  // Tab-separated — Excel pastes this directly into cells
  let tsv = '';

  // Config row
  tsv += `Config\tattendees=${cfg.totalAttendees}\tvip=${(cfg.vipPct*100).toFixed(0)}%\tbags=${(cfg.bagPct*100).toFixed(0)}%\tbagLanes=${cfg.bagLanes}\tarrival=${cfg.arrivalMode}\tbatchSize=${cfg.batchSize}\tbatchInterval=${cfg.batchInterval}s\n\n`;

  // Summary table only
  tsv += `KPI\tMean\t±95% CI\tMin\tMax\tSE\n`;
  for (const s of stats) {
    tsv += `${s.label}\t${s.mean.toFixed(4)}\t${s.hw.toFixed(4)}\t${s.min.toFixed(4)}\t${s.max.toFixed(4)}\t${s.se.toFixed(4)}\n`;
  }

  navigator.clipboard.writeText(tsv).then(() => {
    const btn = document.getElementById('batch-copy');
...
function wireBatchControls() {
  ...
  document.getElementById('batch-export').addEventListener('click', exportBatchCSV);
  document.getElementById('batch-copy').addEventListener('click', copyBatchTSV);

  document.getElementById('batch-run-btn').addEventListener('click', async () => {
    const n = Math.max(10, Math.min(1000, +document.getElementById('batch-n').value || 100));
    const btn = document.getElementById('batch-run-btn');
    const prog = document.getElementById('batch-progress');
    btn.disabled = true;
    document.getElementById('batch-results').innerHTML = '';
    document.getElementById('batch-export').style.display = 'none';
    document.getElementById('batch-copy').style.display = 'none';

    const summary = await runBatch(n, (done, total) => {
      prog.textContent = `${done} / ${total}`;
    });

    prog.textContent = `✓ ${n} runs complete`;
    btn.disabled = false;
