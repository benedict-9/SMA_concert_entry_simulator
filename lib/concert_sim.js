...
function computeTargets() {
  ...
  const dotsPerRow = Math.max(1, Math.floor(bw / dotGap));
  const maxDots    = dotsPerRow * Math.floor(boxH / dotGap);
  ...
  for (const [cxStr, group] of Object.entries(cols)) {
    ...
    shown.forEach((a, i) => {
      ...
      a.tx = (cx - LY.boxW / 2) + pad + r + col * dotGap;
      a.ty = boxTop + r + row * dotGap;
    });
    // hide overflow dots off-canvas
    group.slice(maxDots).forEach(a => { a.tx = null; a.ty = null; });
  }

  // arriving attendees: park off the left edge
  for (const a of attendees) {
